...
//alle ritten worden opgehaald met de datum van morgen en de gebruiker die is aangeklikt
...
if(isset($_POST['submit']))
{
  ...
  if ($ok) {
    ...
    mysqli_query($db, "DELETE FROM ritten WHERE datum = '$datumTomorrow' AND user_id = " . mysqli_escape_string($db, $myId));
...
if(isset($_POST['delete']))
{
  ...
  foreach($checked as $key => $value) {
    mysqli_query($db, "DELETE FROM ritten WHERE datum = '$datumTomorrow' AND id = '$value' AND user_id = " . mysqli_escape_string($db, $myId));
  }
...
<main>
  ...
  <div class="headplanning">
    <h3>Wijzig de planning van: <mark><?= $row['username']; ?></mark> op <mark><?= $datumTomorrowNL; ?></mark> </h3>
  </div>
  <div class="planningform">
    <form name="formplanning" action="<?php $_SERVER['REQUEST_URI']; ?>" method="post" >
      ...
      <?php } ?>
      <input class="deletebutton" type="submit" name="delete" value="Verwijder rit" onClick="deleteRow('dataTablee')" />
